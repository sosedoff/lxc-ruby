...
describe LXC::Container do
  ...
  it 'should exist' do
    subject.stub_lxc('ls') { "app\napp2" }
    subject.exists?.should be_true

    subject.stub_lxc('ls') { "app2\napp3" }
    subject.exists?.should be_false
  end

  it 'returns STOPPED status' do
    subject.stub_lxc('info', '-n', 'app') { fixture('lxc-info-stopped.txt') }
    subject.status.should eq({:state => 'STOPPED', :pid => '-1'})
  end

  it 'returns RUNNING status' do
    subject.stub_lxc('info', '-n', 'app') { fixture('lxc-info-running.txt') }
    subject.status.should eq({:state => 'RUNNING', :pid => '2125'})
  end

  it 'returns the amount of used memory' do
    subject.stub_lxc('cgroup', '-n', 'app', 'memory.usage_in_bytes') { '3280896' }
    subject.memory_usage.should eq('3280896')
  end

  it 'returns the memory limit' do
    subject.stub_lxc('cgroup', '-n', 'app', 'memory.limit_in_bytes') { '268435456' }
